...
.controller('searchJobCtrl', function ($scope, $rootScope, $state, jobService, searchOptions, searchUtil, commonUtil) {
	var vm = this;
	...
	vm.search = function () {
		...
		$state.go($state.current, {page: 1, size: 10, name: vm.data.name, type: null, subType: null}, {reload: true});
	};
	vm.clear = function () {
