...
angular.module('carrot')
  .controller('jobListCtrl', ['$scope', '$rootScope', '$state', 'jobService', 'searchOptions', 'searchUtil', 'commonUtil',
	function ($scope, $rootScope, $state, jobService, searchOptions, searchUtil, commonUtil) {
		...
		vm.search = function () {
			...
			$state.go($state.current, {page: 1, size: 8, name: vm.data.name}, {reload: true});
		};
		//清除
		vm.clear = function () {
			...
			$state.go($state.current, {page: 1, size: 8, name: null}, {reload: true});
		};
